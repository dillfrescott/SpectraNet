...
def inference(model, checkpoint_path, input_wav_path, output_instrumental_path, output_vocal_path,
    ...
    with tqdm(total=num_chunks, desc="Processing audio") as pbar:
        for i in range(0, total_length - chunk_size + 1, chunk_size - overlap):
            ...
            # Reconstruct instrumental and vocal spectrograms
            inst_spec = pred_inst_mag * torch.exp(1j * chunk_phase)
            vocal_spec = pred_vocal_mag * torch.exp(1j * chunk_phase)

            # Ensure spectrograms have the correct shape for istft
            ...
            vocal_chunk = torch.istft(vocal_spec, n_fft=n_fft, hop_length=hop_length, window=window, length=chunk_size, return_complex=False)
    ...
    return instrumentals.cpu(), vocals.cpu()
...
def main():
    ...
    if args.train:
        ...
        if args.input_wav is None:
            ...
            return
        model = UNetCNN(in_channels=2, hidden_size=256, num_layers=args.num_layers)
        instrumentals, vocals = inference(model, args.checkpoint_path, args.input_wav, args.output_instrumental, args.output_vocal, device=device, n_fft=args.n_fft, hop_length=args.hop_length)
        print(f"Instrumental saved to {args.output_instrumental}")
        print(f"Vocal saved to {args.output_vocal}")
    else:
